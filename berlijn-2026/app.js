...
function openInfographic(uid){
  ...
  const history=HISTORY[stop.title] || (stop.type.includes("Route") || stop.type.includes("Praktisch") || stop.type.includes("Start") || stop.type.includes("Eind") ? "Dit is vooral een praktisch routepunt in jullie planning. De waarde zit in de ligging en de logische aansluiting op de volgende stop." : "Deze locatie vertelt een eigen stukje van Berlijn. Gebruik de samenvatting en bezoektips hieronder om snel te bepalen waar jullie aandacht aan willen geven.");
  const lookFor=LOOK_FOR[stop.title] || `Gebruik ${stop.title} niet alleen als fotostop. Kijk naar de omgeving, lees één informatiepaneel en verbind wat je ziet met de plaats van deze stop in de dagroute.`;
  document.getElementById("infographic-root").innerHTML=`<div class="info-overlay" role="dialog" aria-modal="true" aria-labelledby="info-title"><article class="info-sheet"><header class="info-hero">${photo?`<img src="${photo}" alt="${esc(stop.title)}">`:""}<button class="info-close" aria-label="Sluiten">×</button><div class="info-title"><span>${esc(stop.type)} · ${esc(stop.visit)}</span><h2 id="info-title">${esc(stop.title)}</h2></div></header><div class="info-body"><section class="score-card"><div class="score-number">${rating.score?rating.score.toFixed(1):"–"}</div><div><div class="rating-stars-wrap">${stars(rating.score)}</div><strong>${classification(rating.score)}</strong><small>${rating.score?"Tripadvisor-waardering · gecontroleerd 29 juli 2026":"Geen afzonderlijke score vastgezet; open Tripadvisor live"}</small></div><a href="${rating.url}" target="_blank" rel="noopener">Tripadvisor openen</a></section><div class="infographic-grid"><section class="info-block wide story-block"><div class="icon">⌛</div><h3>Historie en betekenis</h3><p>${esc(history)}</p><p>${esc(stop.summary)}</p></section><section class="info-block"><div class="icon">👀</div><h3>Wat gaan jullie precies doen?</h3><p>${esc(stop.details)}</p></section><section class="info-block"><div class="icon">⏱</div><h3>Jullie bezoek</h3><p><strong>${esc(stop.visit)}</strong><br>${esc(stop.inside)}<br>Gepland: ${esc(stop.time)}</p></section><section class="info-block wide accent-block"><div class="icon">🔎</div><h3>Hier moet je op letten</h3><p>${esc(lookFor)}</p></section><section class="info-block wide"><div class="icon">💡</div><h3>Praktische bezoektip</h3><p>${esc(stop.practical)}</p><p>${esc(visitStrategy(stop))}</p></section><section class="info-block wide family-block"><div class="icon">👧</div><h3>Met Layina</h3><p>${esc(familyTip(stop))}</p></section><section class="info-block"><div class="icon">📍</div><h3>Adres</h3><p>${esc(stop.address)}</p></section><section class="info-block"><div class="icon">🧭</div><h3>Plaats in de route</h3><p>Stop ${esc(stop.letter)} op ${esc(stop.time)}. Google Maps bepaalt bij openen de route vanaf jullie actuele locatie.</p></section><section class="info-block wide rating-explain"><div class="icon">★</div><h3>Hoe lees je de Tripadvisor-sterren?</h3><p>4,5–5 sterren is uitzonderlijk sterk, 4,0–4,4 is zeer goed en 3,5–3,9 is goed met meer gemengde ervaringen. Geen ingevulde sterren betekent dat we geen betrouwbare zelfstandige vermelding hebben vastgezet; de knop opent dan de actuele pagina of zoekopdracht.</p></section></div><div class="info-actions"><a href="${navigateNow(stop.address)}" target="_blank" rel="noopener">➤ Navigeer vanaf mijn locatie</a>${stop.official?`<a href="${esc(stop.official)}" target="_blank" rel="noopener">Tickets / officiële website</a>`:`<a href="${mapsSearch(stop.address)}" target="_blank" rel="noopener">Open in Google Maps</a>`}</div></div></article></div>`;
  document.body.classList.add("modal-open");
  document.querySelector(".info-close").focus();
}
